   ...
   Conexion conexion = new Conexion();
        ...
        try {
            ...
            ResultSet result = sql.executeQuery("select a.nombre,a.telefono,a.horario,IFNULL((SELECT bitacoraMKT.observacion FROM bitacoraMKT, "
                    + "clientesPedidos WHERE clientesPedidos.nombre=bitacoraMKT.cliente AND bitacoraMKT.fecha='"+date+"' order by bitacoraMKT.id DESC LIMIT 1),'0') AS 'Observ' from clientesPedidos as a");
            while (result.next()) {
